...
The \textbf{SystemTask} manages the operating mode of the subsystem
and handles user interaction through the tactile button and Dashboard
commands. At each execution, the task checks for button presses and
incoming serial messages to update the system mode. The task can
transition between \texttt{AUTOMATIC} and \texttt{MANUAL} modes
either when the tactile button is pressed or when a mode-switch
command is received from the Dashboard via the communication layer.
The system transitions to \texttt{UNCONNECTED} mode when a network
loss notification (message \texttt{U}) is received via serial
communication from the CUS, signaling that the Tank Monitoring
Subsystem is offline. The task monitors for recovery messages
(message \texttt{C}) to transition back from \texttt{UNCONNECTED} to
\texttt{AUTOMATIC} mode when connectivity is restored. The task
updates the shared system context to reflect the current mode and to
...
that operates the water channel valve. 

\begin{figure}[H]
  \centering
  \includegraphics[width=0.6\textwidth]{./pdf/ValveTask.pdf}
...
timestamp of the last received message. The task also outputs status
...
\begin{Verbatim}[tabsize=4,xleftmargin=2em]
> python control-unit-subsystem/main.py <port>
\end{Verbatim}
