...
\section{Flight Task}
The Flight Task consists of two states: \texttt{WAITING} and
\texttt{OPERATING}. In
\texttt{WAITING} the task waits for a command from the drone (take-off or
landing). When a request is accepted, the task transitions to
\texttt{OPERATING}, where it waits for the operation to complete
(exit detection for take-off, or touchdown detection for landing). Once
the operation completes the task returns to \texttt{WAITING}.
...
\section{Gate Task}
The Gate Task controls the gate with four states: \texttt{CLOSED},
\texttt{OPENING}, \texttt{OPEN}, and \texttt{CLOSING}. At startup the gate is
\texttt{CLOSED}. On the \texttt{open} command it transitions to \texttt{OPENING}
and then to \texttt{OPEN} when the opening is completed. On the
\texttt{close} command it transitions to \texttt{CLOSING} and then to
\texttt{CLOSED} once closed.

If an opposite command arrives while the gate is moving, the
