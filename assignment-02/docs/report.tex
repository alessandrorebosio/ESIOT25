...
\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{./pdf/BlinkingTask.pdf}
  \caption{Blinking Task state diagram.}
  \label{fig:BlinkingTask}
\end{figure}

\newpage
\section{Hangar Control Task}
The state diagram shows the main operating modes of the system:
\texttt{NORMAL}, \texttt{MANAGE}, \texttt{PRE-ALARM}, and
\texttt{ALARM}. Transitions are triggered by commands from the
\texttt{DRU}, sensor events (\texttt{DDD}, \texttt{DPD}), temperature
thresholds being exceeded, and by pressing the \texttt{RESET} button.
Temporal conditions and the configurable parameters (\texttt{D1},
  \texttt{D2}, \texttt{T1}, \texttt{T2}, \texttt{T3}, \texttt{T4},
\texttt{Temp1}, \texttt{Temp2}) are shown on the transition edges.

\begin{figure}[H]
  \centering
  \includegraphics[width=\textwidth]{./pdf/HangarControlTask.pdf}
  \caption{Hangar Control Task state diagram.}
  \label{fig:HangarControlTask}
\end{figure}
...
\begin{Verbatim}[tabsize=4,xleftmargin=2em]
> java -jar drone-hangar-unit-all.jar port baud
\end{Verbatim}

\vspace{\baselineskip}
\noindent Alternatively you can start the project with:
\begin{Verbatim}[tabsize=4,xleftmargin=2em]
> ./gradlew run --args='port baud'
\end{Verbatim}

\vspace{\baselineskip}
\noindent Note: adjust the serial port name for your system (for example, COM3
on Windows or /dev/ttyACM0 on Linux). If the baud rate is not changed
from the default, use 9600. Ensure Java is installed before running the command.
