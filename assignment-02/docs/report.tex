...
\section{Flight Task}
The \textbf{FlightTask} manages take-off and landing operations, acting as the interface 
between system logic and actuator control. It operates through three states: 
\texttt{IDLE}, \texttt{WAITING}, and \texttt{CHECKING}.

At startup, the task begins in \texttt{CHECKING}, verifying sensor data to determine 
whether a flight is in progress. This ensures synchronization between software status 
and the hangar’s physical condition after resets or power loss.

During operation, when a take-off command is received and the drone is inside the 
hangar (\(distance \le D_1\)), the task moves from \texttt{IDLE} to \texttt{CHECKING}. 
Here, the gate opens, LED~L2 blinks, and the distance is monitored to confirm departure.

For a landing command, the system enters \texttt{WAITING}, starts a timer, and waits for 
the PIR sensor to detect the drone. If detection occurs before timeout \(T_5\), it 
transitions to \texttt{CHECKING}; otherwise, it returns to \texttt{IDLE}.

In \texttt{CHECKING}, the task supervises both phases: take-off is confirmed when 
\(distance > D_1\) for longer than \(T_3\), and landing when \(distance < D_2\) for at 
least \(T_4\). Once completed, the gate closes, LED~L2 turns off, and the system returns 
to \texttt{IDLE}.

This unified design enables automatic state recovery and consistent alignment between 
logic, sensors, and the physical hangar.
...
\(D_1\), the task transitions directly to \texttt{CHECKING}. 
