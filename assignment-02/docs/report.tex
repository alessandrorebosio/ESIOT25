...
\section{System Task}
The system has three states: \texttt{NORMAL}, \texttt{PREALARM} and
\texttt{ALARM}.

In \texttt{NORMAL} take-offs and landings are allowed. If temperature
stays at or above \texttt{TEMP1} for more than \texttt{T1}, the
system goes to \texttt{PREALARM}; the timer resets when temperature
drops below \texttt{TEMP1}.

In \texttt{PREALARM} new flights are suspended but ongoing operations
may complete. If temperature stays at or above \texttt{TEMP2} for
more than \texttt{T2}, the system enters \texttt{ALARM}. If
temperature falls below \texttt{TEMP1} it returns to \texttt{NORMAL}.

In \texttt{ALARM} the door is closed and the alarm is activated; if
the drone is outside an alarm message is sent via \texttt{DRU}. The
system stays in \texttt{ALARM} until \texttt{RESET} is pressed, which
...
\section{Flight Task}
The \texttt{FlightTask} manages drone take-off and landing operations
through three main states.  Initially in the \texttt{IDLE} state, the
task awaits user commands.

Upon receiving a take-off command, it verifies the drone's distance
and, if below threshold \texttt{D1},  transitions to the
\texttt{OPERATING} state to execute the departure sequence.

For landing operations, it moves to the \texttt{WAITING} state, where
it awaits drone detection within a predefined timeout \texttt{T5}.

In \texttt{OPERATING}, the hangar door opens and LED L2 starts
blinking; upon operation completion, the door closes, blinking stops,
and the system returns to \texttt{IDLE}.
...
Note: When the Flight Task enters the \texttt{OPERATING} state, it
triggers the \texttt{/startBlink} and \texttt{/openGate} actions to
activate the corresponding tasks. Upon completion of the flight
operation, it executes \texttt{/stopBlink} and \texttt{/closeGate} to
stop these tasks.

\subsection{Flight Task Operation}

Based on sensor checks and conditions, the system then activates
either \texttt{/doLanding} or \texttt{/doTakeoff} to initiate the
specialized landing or takeoff sequences. These dedicated tasks
handle the complete operational workflow and, once finished, signal
the Flight Task to transition from \texttt{OPERATING} back to
\texttt{IDLE} state.
...
\section{Gate Task}
The \texttt{Gate Task} controls the hangar door using four states:
\texttt{CLOSE}, \texttt{OPENING}, \texttt{OPEN}, and
\texttt{CLOSING}. At startup the gate is in \texttt{CLOSE}. When
enabled it goes to \texttt{OPENING} until fully open, then to
\texttt{OPEN}. If enabled becomes false it moves to \texttt{CLOSING}
until \texttt{CLOSE}.

If an opposite command arrives while moving, the task reverses
...
The \texttt{Observer Task} is responsible for periodically monitoring
system conditions through the evaluation of a predicate function.
It operates in a single state, \texttt{OBSERVER}, where at each
activation the associated predicate is checked.
If the predicate evaluates to true, the corresponding function or
callback is executed, allowing the system to react to specific events
or conditions in real time.
This task provides a lightweight mechanism for asynchronous
observation and event detection without interfering with the main control logic.
...
Note: The Observer Task acts as the system's central listener. It
periodically reads inputs and messages (for example from the
DRU/serial) and evaluates the associated predicates without consuming
the messages inside specific functional tasks. When an event or
command is detected, the Observer delegates processing and the
relevant values to the appropriate tasks (System, Flight, Gate,
etc.), keeping each task lightweight and focused on its own logic.

\vspace{\baselineskip}
This design allows multiple SystemTask instances, each with its own
Context. The Observer receives serial messages and forwards each one
to the correct Context, so a single Arduino can manage multiple
hangars. This simplifies message routing and makes the system easier
