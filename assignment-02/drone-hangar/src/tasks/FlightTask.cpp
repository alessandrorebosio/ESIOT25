...
Flight::FlightTask::FlightTask(HWFlight *hw, Context &ctx, MsgService &msg, const bool &enabled, int period)
    : hardware(hw), context(ctx), msg(msg), enabled(enabled), state(nullptr) {
    Task::init(period);
